  ...
  <!-- 注册 -->
		<div class="form-container sign-up">
			<form  @submit.prevent="preRegister" class="register">
				...
				<input v-model="formData.upsw" type="password"  name="upsw" placeholder="请输入密码*（必填）" autocomplete="upsw" title="密码长度在8~20位，只能包含字母和数字"/>

				<input v-model="formData.second_pwd" type="password"  name="second_pwd" placeholder="请再次输入密码*（必填）" autocomplete="second_pwd" title="密码长度在8~20位，只能包含字母和数字"/>

				<input v-model="formData.checkinput" name="checkinput" placeholder="点击下方图片刷新（验证码*：必填）" value="" class="checkinput" >
				<div class="login-code">
					<img :src="codeUrl" @click="getCode" class="register-code-img" alt="验证码图片"/>
				</div>
	...
	methods: {
		getCode(){
			getCodeImg().then(res=>{
				...
				this.codeUrl = "data:image/gif;base64," + res.data.img;	// Base64格式加载验证码
			})
		},
		preRegister(){
